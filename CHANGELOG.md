...
- `inputs` and `outputs` in `Miva.node.ts` now use the string literal `'main'` instead of `NodeConnectionType.Main`. The installed version of `n8n-workflow` exports `NodeConnectionType` as a type only, so referencing it as a value caused a TypeScript compile error.
